class HScrollbar {
  ...
  void update() {
    ...
    if (locked) 
      newSliderPosition = constrain(mouseX  - windowSize/2  - barHeight/2, sliderPositionMin, sliderPositionMax);
   ...
   */
  boolean isMouseOver() {
    if (mouseX  - windowSize/2 > xPosition && mouseX  - windowSize/2 < xPosition+barWidth &&
      mouseY - windowSize/2 > yPosition && mouseY - windowSize/2 < yPosition+barHeight) {
      return true;
    } else {
      return false;
    }
